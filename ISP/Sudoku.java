...
public class Sudoku implements ActionListener, MouseWheelListener
{
 ...
 timerDisplay.setText (hours + ":" + minutes + ":" + seconds);
    ...
    /* Don't need
       ...
       {
    ArrayList[] [] canSolve = new ArrayList [9] [9];
    for (int x = 0 ; x < 9 ; x++)
    {
	// canSolve [x] = new ArrayList [9];
	for (int y = 0 ; y < 9 ; y++)
	{
     canSolve [x] [y] = new ArrayList ();
     for (int z = 1 ; z < 10 ; z++)
     {
	 canSolve [x] [y].add (Integer.toString (z));
    ...
    solvable (missing, canSolve, 81 - difficulty, 0);
    return (true);
       }



    public boolean solvable (int[] [] sGrid, ArrayList[] [] canSolve, int counter, int tries)
    {
	int newCounter = counter;
	int[] [] newGrid = new int [9] [9];
	for (int x = 0 ; x < 9 ; x++)
	{
     newGrid [x] = (int[]) sGrid [x].clone ();
	}
	boolean solved = true;
	for (int x = 0 ; x < 9 ; x++)
	{
	 ...
	 if (newGrid [x] [y] == 0)
      solved = false;
	 if (canSolve [x] [y].size () == 1)
	 {
      numberSet = (String) canSolve [x] [y].get (0);
      newGrid [x] [y] = Integer.parseInt ((String) (canSolve [x] [y].get (0)));
      canSolve [x] [y].clear ();
	...
	for (int x = 0 ; x < 9 ; x++)
	{
	 ...
	 for (int z = 0 ; z < 9 ; z++)
	 {
      if (canSolve [x] [z].contains (Integer.toString (newGrid [x] [y])))
      {
	  canSolve [x] [z].remove (Integer.toString (newGrid [x] [y]));
      }
      if (canSolve [z] [y].contains (Integer.toString (newGrid [x] [y])))
      {
	  canSolve [z] [y].remove (Integer.toString (newGrid [x] [y]));
      }
	 }
     }
	}
	...
	for (int x = 0 ; x < 9 ; x++)
	{
     ...
     for (int y = 0 ; y < 9 ; y++)
     {
	 for (int z = 0 ; z < canSolve [x] [y].size () ; z++)
	 {
      System.out.print (canSolve [x] [y].get (z));
	 }
	 System.out.print (' ');
     }
     System.out.println ();
	...
	if (tries == 10)
	{
     if (randomNumber (newGrid, canSolve, newCounter))
     {
	 boolean done = true;
	 for (int x = 0 ; x < 9 ; x++)
	 {
	...
	if (solvable (newGrid, canSolve, newCounter, tries + 1))
	{
     boolean done = true;
     for (int x = 0 ; x < 9 ; x++)
     {
    ...
    public boolean randomNumber (int[] [] sGrid, ArrayList[] [] canSolve, int counter)
    {
	int newCounter = counter;
	int[] [] newGrid = new int [9] [9];
	for (int x = 0 ; x < 9 ; x++)
	...
	ArrayList[] [] newCanSolve = new ArrayList [9] [9];
	ArrayList[] [] hold = new ArrayList [9] [9];
	for (int x = 0 ; x < 9 ; x++)
	{
     newCanSolve [x] = (ArrayList[]) canSolve [x].clone ();
     hold [x] = (ArrayList[]) canSolve [x].clone ();
	}
	for (int x = 0 ; x < 9 ; x++)
	{
     for (int y = 0 ; y < 9 ; y++)
	 ...
	 if (newGrid [x] [y] == 0)
	 {
      ...
      for (int a = 0 ; a < newCanSolve [x] [y].size () ; a++)
      {
	  newGrid [x] [y] = Integer.parseInt ((String) (newCanSolve [x] [y].get (a)));
	  numberSet = Integer.toString (newGrid [x] [y]);
	  gridButtons [x] [y].doClick ();
	  for (int z = 0 ; z < 9 ; z++)
	  {
       if (newCanSolve [x] [z].contains (Integer.toString (newGrid [x] [y])))
       {
	   newCanSolve [x] [z].remove (Integer.toString (newGrid [x] [y]));
       }
       if (newCanSolve [z] [y].contains (Integer.toString (newGrid [x] [y])))
       {
	   newCanSolve [z] [y].remove (Integer.toString (newGrid [x] [y]));
       }
	  }
	  newCanSolve [x] [y].clear ();
	  if (solvable (newGrid, newCanSolve, newCounter + 1, 0))
	  {
       return (true);
	  }
	  newCanSolve [x] [y] = hold [x] [y];
	  for (int z = 0 ; z < 9 ; z++)
	  {
       newCanSolve [x] [z] = hold [x] [z];
       newCanSolve [z] [y] = hold [z] [y];
	  }
      }
	 }
     }
 ...
 if (runType == 0)
 {
     ...
     //System.out.println (saveFiles.length);
 }
 ...
 */
 ...
 try
 {
  ...
  if (scores [x] >= time || scores [x] == 0)
  {
   ...
   if (!name.equals ("") && name != null && name.length () < 61)
   {
       break;
   }
 ...
 else if (temp == introToMainMenu)
 {
     ...
     //System.out.println (introPanel.isFocused ());
